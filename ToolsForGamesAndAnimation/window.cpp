...
Window::Window(int width, int height, int windowTopLeftX, int windowTopLeftY, const std::string& title, HINSTANCE hInstance, int iCmdShow) {
	...
	m_winWindowClass.cbClsExtra = 0;											// no idea
	m_winWindowClass.cbWndExtra = 0;											// no idea
	m_winWindowClass.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);	// background fill black
	m_winWindowClass.hCursor = LoadCursor(NULL, IDC_ARROW);					// arrow cursor       
	m_winWindowClass.hIcon = LoadIcon(NULL, IDI_APPLICATION);				// type of icon to use (default app icon)
	m_winWindowClass.hInstance = hInstance;									// window instance name (given by the OS when the window is created)   
	m_winWindowClass.lpfnWndProc = WndProc;									// window callback function - this is our function below that is called whenever something happens
	m_winWindowClass.lpszClassName = TEXT("my window class name");				// our new window class name
	m_winWindowClass.lpszMenuName = 0;											// window menu name (0 = default menu?) 
	m_winWindowClass.style = CS_HREDRAW | CS_VREDRAW | CS_OWNDC;				// redraw if the window is resized horizontally or vertically, allow different context for each window instance

																				// Register that class with the Windows OS..
	RegisterClass(&m_winWindowClass);
	...
	m_winWindow = CreateWindow(TEXT("my window class name"),		// window class to use - in this case the one we created a minute ago
	...
	ShowWindow(m_winWindow, iCmdShow);

	// get a device context from the window
	m_DeviceContext = GetDC(m_winWindow);
...
Window::~Window() {
	...
	ReleaseDC(m_winWindow, m_DeviceContext);
